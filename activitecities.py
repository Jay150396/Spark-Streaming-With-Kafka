...
city_df = us_df.select(col("data.group.group_city").alias("topic"))

# Creating Spark SQL Table In Memory(RAM)
sql = city_df.writeStream.outputMode("append").format('memory').queryName('activeCities').start()

# Runing Spark SQL quries on the table
output = spark.sql("select topic, count(*) as topic_count from activeCities group by topic order by topic_count desc")
...
output_df.write.format("kafka").option("kafka.bootstrap.servers", "localhost:9092").option("topic", "activeCitiesOutput").save()
